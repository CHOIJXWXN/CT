...
for i in range(N - 1):
    ...
    tmp = maxL[:]
    maxL[0] = max(tmp[0], tmp[1]) + L[0]
    maxL[1] = max(tmp[0], tmp[1], tmp[2]) + L[1]
    maxL[2] = max(tmp[1], tmp[2]) + L[2]
    tmp = minL[:]
    minL[0] = min(tmp[0], tmp[1]) + L[0]
    minL[1] = min(tmp[0], tmp[1], tmp[2]) + L[1]
    minL[2] = min(tmp[1], tmp[2]) + L[2]
